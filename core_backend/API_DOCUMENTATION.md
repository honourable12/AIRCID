...
This document provides a detailed overview of the Core Backend API, including its purpose, setup instructions, and comprehensive endpoint documentation.
...
### Retrieving Related Data (Include)

Many endpoints support the `include` query parameter to retrieve related data in the same request. This is useful for reducing the number of API calls needed to get all the information for a specific resource.

You can specify which related models to include by passing them as a comma-separated list in the `include` parameter.

**Example: Get a study and include its forms and questions**

```bash
curl -X GET "http://127.0.0.1:8000/api/v1/studies/1?include=forms,forms.questions" -H "Authorization: Bearer <your-token>"
```

**Example: Get a form and include its questions and the study it belongs to**

```bash
curl -X GET "http://127.0.0.1:8000/api/v1/forms/1?include=questions,study" -H "Authorization: Bearer <your-token>"
```
